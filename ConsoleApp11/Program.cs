...
namespace ConsoleApp9
{
    class Element : IComparable<Element>
    {
        ...
        {
            return String.Compare(this.name, element.name);
        }
    }
    ...
    class Combine : Element, IComparable<Combine>
    {
        ...
        private List<bool> numInput = new List<bool>();
        //констуктор по умолчанию
        public Combine() : base("ИЛИ-НЕ",5,1)
        {
            //for (int i = 0; i < 5; i++) setInput(i + 1, false);
            for (int i = 0; i < 5; i++) numInput.Add(false);
        }
        ...
        public void setNumInput(bool[] numInput)
        {
            try
            {
                for (int i = 0; i < 5; i++) this.numInput.Add(numInput[i]);
            }
        ...
        //переопределнный compare
        public int CompareTo(Combine comb)
        {
            return String.Compare(this.Name, comb.Name);
        }
    }
    ...
    {
        //триггер
        class Memory : Element, IComparable<Memory>
        {
            //private bool[] memIn;
            private List<bool> memIn = new List<bool>();
            ...
            public Memory()
            {
                ...
                Input = 2;
                Output = 1;
                for (int i = 0; i < Input; i++) memIn.Add(false);
            }
            ...
            public Memory(bool memOutDir, bool memOutInv, bool[] memIn)
            {
                try
                {
                    ...
                    for (int i = 0; i < 2; i++) this.memIn.Add(memIn[i]);
                }
            ...
            //переопределнный compare
            public int CompareTo(Memory mem)
            {
                if (this.OutDir == mem.OutDir) return 0;
                else if (this.OutDir) return 1;
                else return -1;
            }
        }
        private bool r, s;
        ...
        {
            if (this.r == reg.r) return 0;
            else if (this.r) return 1;
            else return -1;
        }
    }
    class Program
